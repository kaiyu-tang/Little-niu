...
class QiniuMacAuth(object):
    ...
    def __token(self, data):
        data = data
        hashed = hmac.new(bytes(self.__secret_key, encoding='utf8'), bytes(data, encoding='utf8'), sha1)
        return bytes.decode(urlsafe_base64_encode(hashed.digest()))
